import { EvaluationMetrics, RuleSet, RuleViolation, RuleType } from './types';
...
const maxDrawdownValidator: Validator = (metrics, rules) => {
  if (rules.maxDrawdown === undefined) return null;
...
const dailyLossValidator: Validator = (metrics, rules) => {
  if (rules.dailyLossLimit === undefined) return null;
...
const leverageValidator: Validator = (metrics, rules) => {
  if (rules.leverageLimit === undefined) return null;
...
export class RuleValidatorRegistry {
  ...
  checkPassConditions(metrics: EvaluationMetrics, rules: RuleSet): boolean {
    ...
    const daysMet =
      rules.minTradingDays === undefined ||
      metrics.tradingDays >= rules.minTradingDays;
